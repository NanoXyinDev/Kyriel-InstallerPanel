...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try { const res = await register(username, email, password); if (res.error) setError(res.error); else { setSuccess('Akun berhasil dibuat! Redirecting...'); setTimeout(()=>window.location.href='/', 2000) } }
    catch { setError('Gagal konek. Cek network lu.') } finally { setLoading(false) }
  }
    ...
    <div className="min-h-screen grid-bg flex items-center justify-center px-4 relative overflow-hidden safe-bottom">
      ...
      <div className="w-full max-w-sm md:max-w-md relative z-10">
        <div className="text-center mb-6 md:mb-8">
          ...
          <h1 className="text-2xl md:text-3xl font-bold glow-cyan mb-2">KYRIEL<span className="text-kyriel-accent">INSTALLER</span></h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-3 md:space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" value={username} onChange={e=>setUsername(e.target.value)} className="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2.5 pl-10 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-kyriel-cyan/50" placeholder="username123" required minLength={3} />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" value={email} onChange={e=>setEmail(e.target.value)} className="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2.5 pl-10 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-kyriel-cyan/50" placeholder="boss@example.com" required />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <input type="password" value={password} onChange={e=>setPassword(e.target.value)} className="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2.5 pl-10 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-kyriel-cyan/50" placeholder="••••••••" required minLength={6} />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500" />
                <input type="password" value={confirmPass} onChange={e=>setConfirmPass(e.target.value)} className="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2.5 pl-10 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-kyriel-cyan/50" placeholder="••••••••" required />
              </div>
            </div>
            <button type="submit" disabled={loading} className="w-full bg-kyriel-cyan hover:bg-kyriel-cyan/90 text-black font-semibold py-2.5 rounded-lg flex items-center justify-center gap-2 transition-colors disabled:opacity-50 text-sm md:text-base">
