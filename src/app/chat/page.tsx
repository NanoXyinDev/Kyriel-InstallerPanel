...
interface ChatMessage { id: string; username: string; message: string; createdAt: string }
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try { const res = await fetch('/api/chat'); const data = await res.json(); if (data.messages) setMessages(data.messages) }
      catch {}
    }
    fetchMessages()
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.message) { setMessages(prev => [...prev, data.message]); setInput('') }
    } catch {}
    setSending(false)
    ...
    <div className="min-h-screen bg-kyriel-dark safe-bottom">
      ...
      <main className="pt-16 lg:pt-20 pb-20 lg:pb-12 px-3 md:px-4 lg:px-8 max-w-7xl mx-auto h-screen flex flex-col">
            ...
            return (
              <div key={msg.id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
                <div className="flex flex-col max-w-[85%] sm:max-w-[75%] md:max-w-[70%]">
                  {showTime && <div className="flex items-center justify-center gap-1 my-2 md:my-3"><Clock className="w-3 h-3 text-gray-600" /><span className="text-[10px] md:text-xs text-gray-600">{formatTime(msg.createdAt)}</span></div>}
                  <div className="flex items-end gap-1.5 md:gap-2">
                    {!isOwn && <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-gradient-to-br from-kyriel-cyan to-kyriel-purple flex items-center justify-center shrink-0 text-[10px] md:text-xs font-bold text-white">{msg.username.charAt(0).toUpperCase()}</div>}
                    <div className="flex flex-col">
                      {!isOwn && <span className="text-[10px] md:text-xs text-gray-500 mb-0.5 md:mb-1 ml-1">{msg.username}</span>}
                      <div className={`chat-bubble ${isOwn ? 'chat-bubble-own' : 'chat-bubble-other'}`}>{msg.message}</div>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <form onSubmit={sendMessage} className="mt-3 md:mt-4 flex gap-2 md:gap-3">
          <input ref={inputRef} type="text" value={input} onChange={e=>setInput(e.target.value)} placeholder="Ketik pesan..." className="input-dark flex-1" maxLength={500} />
          <button type="submit" disabled={sending || !input.trim()} className="btn-primary px-3 md:px-5 flex items-center gap-1.5 md:gap-2 disabled:opacity-50">
            <Send className="w-4 h-4" /><span className="hidden sm:inline text-sm">Kirim</span>
          </button>
        </form>
      </main>
    </div>
