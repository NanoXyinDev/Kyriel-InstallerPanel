...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!message || message.trim().length === 0) return NextResponse.json({ error: 'Message empty.' }, { status: 400, headers: getSecurityHeaders() })
    if (message.length > 500) return NextResponse.json({ error: 'Message too long (max 500 chars).' }, { status: 400, headers: getSecurityHeaders() })
    const newMsg = await sendChatMessage({ userId: decoded.id, username: decoded.username, message: message.trim() })
