...
import { getLogsByUser, getLogById } from '../../../lib/github-db'
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (logId) {
      ...
      if (!log || log.userId !== decoded.id) return NextResponse.json({ error: 'Log not found.' }, { status: 404, headers: getSecurityHeaders() })
      return NextResponse.json({ log }, { headers: getSecurityHeaders() })
    }
    const logs = await getLogsByUser(decoded.id)
    return NextResponse.json({ logs }, { headers: getSecurityHeaders() })
  } catch {
